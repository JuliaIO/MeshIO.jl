...
immutable Mime
	...
	function Mime(unique_identifier::Symbol, ending::Symbol, magic_number::Vector{Uint8}=b"")
		...
		if isempty(magic_number) 
			if !isempty(candidate_list) && !isempty(mimes_with_no_magic)
				error(
					"Ending ($ending) is not unique and some Mime with the same ending has also no magic number. 
					Conflicting Mime: $mimes_with_no_magic. 
					Please find some better differentiation feature"
				)
			end
	...
	function Mime(io::IO, path::String)
		...
		i = 1
		for j=1:maximum_length
        	eof(io) && break
        	char = read(io, Uint8)
        	char == Uint8('\r') && continue # Windows line breaks have to be handled. Is this really bad? 
        	magicbuf[i] = char
        	i+=1
    	end
...
function mem_compare(a::Vector{Uint8}, b::Vector{Uint8}, len=min(length(a), length(b)))
	0 == ccall(:memcmp, Int32, (Ptr{Uint8}, Ptr{Uint8}, Int), a, b, len)
end

File(segment1::AbstractString, segment2::AbstractString, path_segments::AbstractString...) = File(joinpath(segment1, segment2, path_segments...))

macro file_str(path::AbstractString)
	File(path)
end

Base.open(f::File) = open(f.path)
Base.abspath(f::File) = f.path

load(f::File; options...) 								= load(f, Mime(f); options...)
save(data, f::File; mime=defaultmime(data), options...) = save(data, f; mime=mime, options...)
...
function test()
	a=file"test_ascii.ply"
	b=file"test_binary.ply"
	c=file"bunny.ply"
